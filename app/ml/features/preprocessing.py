...
if __name__ == "__main__":
    ...
    caminho_csv = os.path.join(BASE_DIR, 'app', 'ml', 'features', 'dados_transformados_V2.csv')
    x_transformado.to_csv(caminho_csv, index=False)
    print(f"\n📊 CSV de treino salvo em: {caminho_csv}")

    caminho_pipeline = os.path.join(BASE_DIR, 'app', 'ml', 'features', 'pipeline_limpeza_V2.pkl')
